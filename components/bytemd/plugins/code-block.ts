...
const successTip = `<span style="font-size: 12px;">Copy Success!</span>`;
  ...
  return {
    ...
    viewerEffect({ markdownBody }) {
      ...
      for (const element of elements) {
        // Click button to copy code to clipboard
        element.addEventListener('click', () => {
          let codeText = element.textContent ?? '';
          // Remove the $ symbol at the beginning, then trim it, usually used for shell command code blocks
